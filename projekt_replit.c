...
void o(int *point_file_read, int *point_arrays_allocated,char ***nazvyPrispevkov, char ***menaAutorov, char ***typyPrezentovania, char ***casyPrezentovania, char ***datumy, FILE **fr){
    ...
    while(getchar() != '\n');
    if(*point_arrays_allocated){
        int *uIndexes, *pIndexes;
        int uPos = 0, pPos = 0;

        // Allocates memory for arrays containing indexes for each presentation type category based on number of allocated arrays
        uIndexes = (int *) malloc(*point_arrays_allocated * sizeof(int));
        pIndexes = (int *) malloc(*point_arrays_allocated * sizeof(int));
        ...
        for(int i = 0; i < *point_arrays_allocated; i++){
            ...
            if(helper == datum){
                match_found = 1;
                if((*typyPrezentovania)[i][0] == 'U'){
                    uIndexes[uPos] = i;
                    uPos++;
                }
                else if((*typyPrezentovania)[i][0] == 'P'){
                    pIndexes[pPos] = i;
                    pPos++;
                }
            }
        ...
        for(int i = 0; i < uPos-1; i++){
            for(int j = 0; j < uPos - 1 - i; j++){
                helper = (int) atoi((*casyPrezentovania)[uIndexes[j]]);
                pos_switch_helper = (int) atoi((*casyPrezentovania)[uIndexes[j+1]]);
                if(helper > pos_switch_helper){
                    pos_switch_temp = uIndexes[j];
                    uIndexes[j] = uIndexes[j+1];
                    uIndexes[j+1] = pos_switch_temp;
                }
            }
        }
        for(int i = 0; i < pPos-1; i++){
            for(int j = 0; j < pPos - 1 - i; j++){
                helper = (int) atoi((*casyPrezentovania)[pIndexes[j]]);
                pos_switch_helper = (int) atoi((*casyPrezentovania)[pIndexes[j+1]]);
                if(helper > pos_switch_helper){
                    pos_switch_temp = pIndexes[j];
                    pIndexes[j] = pIndexes[j+1];
                    pIndexes[j+1] = pos_switch_temp;
                }
            }
        }
        // Prints output
        for(int i = 0; i < uPos; i++){
            printf("%s %s %s %s\n",(*casyPrezentovania)[uIndexes[i]], (*typyPrezentovania)[uIndexes[i]], (*menaAutorov)[uIndexes[i]], (*nazvyPrispevkov)[uIndexes[i]]);
        }
        printf("\n");
        for(int i = 0; i < pPos; i++){
            printf("%s %s %s %s\n",(*casyPrezentovania)[pIndexes[i]], (*typyPrezentovania)[pIndexes[i]], (*menaAutorov)[pIndexes[i]], (*nazvyPrispevkov)[pIndexes[i]]);
        }
    ...
    if(*point_file_read){
        ...
        uIndexes = (int *) malloc(tempCount * sizeof(int));
        pIndexes = (int *) malloc(tempCount * sizeof(int));
        ...
        for(int i = 0; i < uPos-1; i++){
            for(int j = 0; j < uPos - 1 - i; j++){
                helper = (int) atoi((casy)[uIndexes[j]]);
                pos_switch_helper = (int) atoi((casy)[uIndexes[j+1]]);
                if(helper > pos_switch_helper){
                    pos_switch_temp = uIndexes[j];
                    uIndexes[j] = uIndexes[j+1];
                    uIndexes[j+1] = pos_switch_temp;
        ...
        for(int i = 0; i < pPos-1; i++){
            for(int j = 0; j < pPos - 1 - i; j++){
                helper = (int) atoi((casy)[pIndexes[j]]);
                pos_switch_helper = (int) atoi((casy)[pIndexes[j+1]]);
                if(helper > pos_switch_helper){
                    pos_switch_temp = pIndexes[j];
                    pIndexes[j] = pIndexes[j+1];
                    pIndexes[j+1] = pos_switch_temp;
        ...
        // Prints output
        for(int i = 0; i < uPos; i++){
            printf("%s %s %s %s\n",(casy)[uIndexes[i]], (typy)[uIndexes[i]], (mena)[uIndexes[i]], (nazvy)[uIndexes[i]]);
        }
        printf("\n");
        for(int i = 0; i < pPos; i++){
            printf("%s %s %s %s\n",(casy)[pIndexes[i]], (typy)[pIndexes[i]], (mena)[pIndexes[i]], (nazvy)[pIndexes[i]]);
        }
        // Freeing mallocated index arrays
        free(uIndexes);
        free(pIndexes);

        // Frees temporary arrays holding file data
        for(int i = 0; i < tempCount; i++){
            free(nazvy[i]);
            free(mena[i]);
            free(typy[i]);
